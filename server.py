...
def enviar(conexao, tipo, texto):
    """Envia um pacote JSON terminado em newline."""
    pacote = json.dumps({"type": tipo, "text": texto}, ensure_ascii=False) + "\n"
    conexao.sendall(pacote.encode())


def receber_linha(conexao, buf):
    """Lê até encontrar '\\n', retorna (linha, buf_restante)."""
    while "\n" not in buf:
